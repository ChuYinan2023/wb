...
app.options('/.netlify/functions/extract-summary', cors(corsOptions));
...
// 添加摘要提取处理程序
app.post('/.netlify/functions/extract-summary', async (req, res) => {
  console.log('收到摘要提取请求，完整请求体:', JSON.stringify(req.body, null, 2));
...
网页内容：${mainText}`
      ...
      console.error('AI 摘要提取失败:', aiError);
      return res.status(500).json({ 
        error: 'Failed to extract summary with AI', 
        details: aiError.message,
        fullError: aiError
      });
    }
